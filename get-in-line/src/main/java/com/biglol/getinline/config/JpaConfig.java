...
@Configuration
public
class JpaConfig { // 오로지 jpa auditing기능 추가하고 싶어서 넣은거임 (JPA Auditing은 엔티티의 생성, 수정 시간 등을 자동으로 관리해주는
    // 기능입니다)
    //
    //    // application.properties에서 jpa관련 설정을 다 끝냈기에 아래 소스는 필요없지만 필요에 따라 직접 세팅이 필요할 때. 아래 코드는
    // application.properties에서 설정한 것과 같음
    ...
    ////        return
    // DataSourceBuilder.create().driverClassName().type().url().username().password().build();
    //
    ////        // (*1) 아니면 이렇게 간단하게 하고 @ConfigurationProperties("spring.datasource")를 설정하여
    // application.properties에 설정한 property들이 주입되게끔 함
    ...
    //
    //    @Bean
    //    public LocalContainerEntityManagerFactoryBean entityManagerFactory() { // 또는 (DataSource
    // dataSource)도 가능 dataSource가 무사히 Bean으로 등록되었을 경우
    ...
    //    // (*2)
    ////    @Bean
    ////    public ChainedTransactionManager chainedTransactionManager(PlatformTransactionManager
    // transactionManager) { // 여러개 넣기 가능. 중요하고 실패하는 것에 대해 좀더 민감한 트랜잭션을 뒤에 배치해야 함
