...
class ErrorCodeTest {
    ...
    @ParameterizedTest(
            name = "[{index}] {0} ===> {1}") // ErrorCode가 많아질 수록 각 케이스에 대한 테스트를 다 입력할 수 없기에 아래처럼
    // 작성. 이 annotation은 테스트할 때 표시될 테스트명
    @MethodSource // 입력 소스로 method를 쓰겠다. 그 method일므을 사용하는 테스트 대상 테스트 이름과 동일하게 하면 됨 (static으로 되어있는 것)
    ...
    static Stream<Arguments> givenExceptionWithMessage_whenGettingMessage_thenReturnsMessage() {
        return Stream.of(
                arguments(
                        ErrorCode.OK,
                        "Ok - This is test message."), // ErrorCode.OK가 들어가면 "Ok - This is test
                // message." 출력이 나와야 한다고 이해
                arguments(ErrorCode.BAD_REQUEST, "Bad request - This is test message."),
                arguments(
                        ErrorCode.SPRING_BAD_REQUEST,
                        "Spring-detected bad request - This is test message."),
                ...
                arguments(
                        ...
                        "Spring-detected internal error - This is test message."),
                arguments(
                        ErrorCode.DATA_ACCESS_ERROR, "Data access error - This is test message."));
    }
