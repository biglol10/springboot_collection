...
class EventRepositoryTest {
    ...
    public EventRepositoryTest(@Autowired EventRepository sut, @Autowired TestEntityManager testEntityManager) {
        this.sut = sut;
    ...
    void test() {
        ...
        // When
        Iterable<Event> events = sut.findAll(new BooleanBuilder()); // Predicate이 interface라서 그대로 넣어줄 수 없고 Predicate을 구현하는 걸로 BooleanBuilder사용. true false로 where 조건의 결과를 리턴하는 건데 where 절의 결과가 true다 false다 이걸 만들 때 BooleanBuilder사용. 지금은 검색어를 넣지 않으니 가능
...
//        ReflectionTestUtils.setField(place, "id", 1L); // ReflectionTestUtils는 테스트 도구 중에 지원해주는 거임. test중에 쓰고 reflection을 도와줌
        // entity안에 id는 setter을 가지고 있지 않음. 영속성 context를 사용하지 않고 테스트 해야 할 때 저 부분이 null인 상태로 테스트를 해야되는 문제가 발생.
